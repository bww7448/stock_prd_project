...
nsq_p = call_nasdaq()['change']

# 나스닥지수 라벨링
def labellingNASDAQ(n):
    # nsq_p = dsq_data['change']
    if nsq_p[n] > 0.0003:
        if nsq_p[n]< 0.0010:
            return 'U01'
        if nsq_p[n] < 0.0017:
...
def load_stock_with_labels():
    ...
    nq = fdr.DataReader('IXIC', '2012-01-01', today)
    nq.columns = pd.Index(
        ["close", "open", "high", "low", "volume", "change"], name=stockData.columns.name)
    ...
    for i in range(len(stockData)):
        stockData['nasdaq'].values[i] = labellingNASDAQ(i+1)
