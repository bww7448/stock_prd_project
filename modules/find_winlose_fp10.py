...
for f_loss in range(0,10) :
    ...
    for s_profit in range(0,10) :
        ...
        for s_loss in range(0,10) :
            ...
            win_lose_table = win_lose_test(zzin_df = "best_100_df_5", first_profit = 0.1, first_loss = fir_loss, second_profit = sec_profit, second_loss = sec_loss, third_profit = 0.01)
            if win_lose_table['real_profit'].sum() > sum_real_profit :
                sum_real_profit = win_lose_table['real_profit'].sum()
                win_lose_table.to_csv("resources/fp10winlose_table_5.csv", encoding = 'euc-kr')
