...
sum_real_profit = 0
for f_loss in range(5,15) :
    fir_loss = f_loss*0.001
    for s_profit in range(5,15) :
        sec_profit = s_profit*0.001 + 0.07
        for s_loss in range(5,15) :
            sec_loss = s_loss*0.001 + 0.01
            win_lose_table = win_lose_test(zzin_df = "best_100_df", first_profit = 0.08, first_loss = fir_loss, second_profit = sec_profit, second_loss = sec_loss, third_profit = 0.01)
            if win_lose_table['real_profit'].sum() > sum_real_profit :
                sum_real_profit = win_lose_table['real_profit'].sum()
                win_lose_table.to_csv("resources/fp8winlose_table.csv", encoding = 'euc-kr')
